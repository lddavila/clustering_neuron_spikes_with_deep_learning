function [] = train_simple_group_or_dont_wf_normalized(varargin)
...
%rescale the waveforms between 0-1 
for i=1:size(training_assembled_data{2},2)
    training_assembled_data{2}(i,:) = rescale(training_assembled_data{2}(i,:));
end
for i=1:size(training_assembled_data{2},2)
    val_assembled_data{2}(i,:) = rescale(val_assembled_data{2}(i,:));
end
...
for i=1:size(test_assembled_data{2},2)
