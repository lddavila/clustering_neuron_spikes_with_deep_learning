function [] = train_ch_bttr_through_feature_dropping_in_parallel(varargin)
%this differs from
%train_nn_to_find_ideal_cutting_threshold_with_incremental_data.m because
%it runs the leave one out permutations in parallel 
...
%format the needed variables to be parallel constants
training_features_array = parallel.pool.Constant(training_features_array);
val_features_array= parallel.pool.Constant(val_features_array);
test_features_array = parallel.pool.Constant(test_features_array);
training_all_comparisons = parallel.pool.Constant(training_all_comparisons);
val_all_comparisons = parallel.pool.Constant(val_all_comparisons);
test_all_comparisons = parallel.pool.Constant(test_all_comparisons);
train_true_class  = parallel.pool.Constant(train_true_class);
val_true_class  = parallel.pool.Constant(val_true_class);
test_true_class = parallel.pool.Constant(test_true_class);
disp("Finished setting vars for parallel.")
...
parfor i=2:length(table_of_feature_combos)
    ...
    [current_net,current_accuracy] = repeatable_training_set_up(training_features_array.Value, ...
        val_features_array.Value, ...
        test_features_array.Value, ...
        training_all_comparisons.Value, ...
        val_all_comparisons.Value, ...
        test_all_comparisons.Value,1, ...
        current_features,train_true_class.Value,val_true_class.Value,test_true_class.Value);
