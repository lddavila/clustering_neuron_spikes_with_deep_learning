function [] = train_most_simplified_group_or_dont_nn_with_wf_rescale(varargin)
...
%we'll rescale all the mean waveform values between 0-1 to hopefully have a
%more predictable euclidean distance
...
dir_to_save_results_to = fullfile(parent_save_dir,"simplest_group_or_dont");
...
val_is_not_same_idxs = val_comparisons_with_class_0(randperm(length(val_comparisons_with_class_0),number_of_comparisons_per_class));
...
validation_data = [validation_overlap,val_euc_distance_between_rep_wires,val_euc_distance_between_rep_wfs,validation_left_col_size,validation_right_col_size];
