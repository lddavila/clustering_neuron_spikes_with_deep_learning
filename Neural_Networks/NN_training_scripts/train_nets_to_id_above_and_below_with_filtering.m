function [] = train_nets_to_id_above_and_below_with_filtering(varargin)
...
for i=1:length(thresholds)
    ...
    %every neural network that follows the original should only be trained
    %on data that would pass the of the first
    if i~=1
        net_struct = importdata("above_below_"+string(thresholds(i-1))+".mat");
        col_max_filter = net_struct.InputMax;
        col_min_filter = net_struct.InputMin;
        filter_net = net_struct.net;
        all_grades_from_bp_table = assemble_data_for_neural_net(list_of_features_to_add,blind_pass_table,config);
        all_grades_from_bp_table = all_grades_from_bp_table{1};
        %rescale the grades
        all_grades_from_bp_table = rescale(all_grades_from_bp_table,-1,1,"InputMax",col_max_filter,"InputMin",col_min_filter);
