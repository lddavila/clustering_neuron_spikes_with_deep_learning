function [] = train_ch_bttr_with_new_grades_perms(varargin)
%this function takes the additional step of trying to find the ideal set of
%variables to maximize the performance of a choose-better net
%this is different from train_ch_bttr_with_new_grades.m because that
%version automatically uses all grades
%though all grades are useful the hope is to reduce to the most useful
%grades (cores of decision making)
%
...
dir_to_save_results_to = create_a_file_if_it_doesnt_exist_and_ret_abs_path(fullfile(config.parent_save_dir,"ch_bttr_with_valley_perms"));
...
%start at 2 cause no 1 feature is gonna be so predictive
for i=2:length(list_of_feature_nums)-1
    cell_array_of_feature_combos{i} = nchoosek(1:length(list_of_feature_nums),i);
end
...
for k=1:length(cell_array_of_feature_combos)
    current_combos = cell_array_of_feature_combos{k};
    if isempty(current_combos)
        continue;
    end
    for p=1:size(current_combos,1)
        current_feature_combos = current_combos(p,:);
        ...
        %set up storage for the preserved examples
        ...
        for i=1:length(curriculum_thresholds)
            ...
            training_data = [training_features_array(training_comparisons(:,1),current_feature_combos),training_features_array(training_comparisons(:,2),current_feature_combos)];
            val_data = [val_features_array(val_comparisons(:,1),current_feature_combos),val_features_array(val_comparisons(:,2),current_feature_combos)];
            test_data = [test_features_array(test_comparisons(:,1),current_feature_combos),test_features_array(test_comparisons(:,2),current_feature_combos)];
            ...
            par_save("difference_in_stage_"+string(i)+"_test_acc_"+sprintf('%f',accuracy)+"_k_"+string(k)+"_p_"+string(p)+".mat",net);
        end
    end
end
end