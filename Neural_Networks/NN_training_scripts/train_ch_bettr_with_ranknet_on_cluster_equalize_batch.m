function [] = train_ch_bettr_with_ranknet_on_cluster_equalize_batch(varargin)
% Train a Siamese RankNet-style model to choose the better cluster.
% Uses scalar score s(x) and P(x1 > x2) = sigmoid(s(x1) - s(x2)).
%this method is different because every twin batch has an even distribution
%of training
% --- Path & config ---
...
[training_all_comparisons,train_true_class,train_bucket] = sample_data_by_difficulty_bucket(training_all_comparisons,train_true_class,train_bucket);
[val_all_comparisons,val_true_class,val_buckets] = sample_data_by_difficulty_bucket(val_all_comparisons,val_true_class,val_buckets);
...
[test_all_comparisons,test_true_class,test_buckets] = sample_data_by_difficulty_bucket(test_all_comparisons,test_true_class,test_buckets);
...
% --- Nested helper functions ---
    ...
    function [X1,X2,pair_labels] = getTwinBatch(all_combinations, mini_batch_size, true_combination_labels, features,unique_difficulty_classes,diff_class_of_curr_data)
        ...
        %we'll enforce that every bucket equally represents every accuracy
        %class to ensure that the neural network does not prioritize easy
        %comparisons

        %first determine how many of each class should appear in the mini
        %batch
        per_class_count = mini_batch_size / length(unique_difficulty_classes);
        ...
        for p=1:length(unique_difficulty_classes)
            ...
            idx0 = find(~subset_of_combination_labels);
            idx1 = find(subset_of_combination_labels);
            for ii = 1:per_class_count
                choice = rand();

                if choice < 0.5
                    % sample a "left not better" pair (label 0)
                    random_index = idx0(randi(numel(idx0)));
                    random_index = location_before_mask(random_index);
                    pair_labels(position_counter) = 0;
                else
                    % sample a "left better" pair (label 1)
                    random_index = idx1(randi(numel(idx1)));
                    random_index = location_before_mask(random_index);
                    pair_labels(position_counter) = 1;
                end

                pair_idx_1 = all_combinations(random_index,1);
                pair_idx_2 = all_combinations(random_index,2);

                X1(:,position_counter) = single(features(pair_idx_1,:)).';
                X2(:,position_counter) = single(features(pair_idx_2,:)).';
