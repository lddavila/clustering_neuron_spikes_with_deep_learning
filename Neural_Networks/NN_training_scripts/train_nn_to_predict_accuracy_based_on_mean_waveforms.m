...
which_nn = "waveform_only_acc_cats";
...
dir_to_save_results_to = fullfile(parent_save_dir,"waveform_only_acc_cats");
...
filter_sizes = 5:5:20;
what_combinations_of_waveforms_to_add = 1:size(table_of_all_waveforms,1);
...
permutations_table = get_table_of_all_permutations_for_nn_training(["num_acc_cats", ...
    ...
    "filter_sizes", ...
    "which_waveforms"], ...
    number_of_accuracy_categories,number_of_layers,filter_sizes,what_combinations_of_waveforms_to_add);
...
for j=number_of_accuracy_categories
    ...
    for i=1:size(subset_of_perms_table,1)
        %first check which waveforms should be used
        which_waveforms_row = subset_of_perms_table{i,"which_waveforms"};
        which_waveform_combination = table_of_all_waveforms{which_waveforms_row,"permutations_of_waveforms"}{1};

        waveform_data = horzcat(assembled_data{which_waveform_combination});
        ...
        training_set_data = array2table([waveform_data,table_with_accuracy{:,"accuracy_category"}]);
