function [] = train_prob_dist_nn_two_grades(grade_pair,varargin)%the goal of this function is to use the neural network thresholding idea
...
load('/Users/justin/Downloads/SpikeSorting_Project/blind_pass_table.mat')
...
config.NAMES_OF_CURR_GRADES = config.NAMES_OF_CURR_GRADES(1:63);
config.expanded_grade_idxs = config.expanded_grade_idxs(config.expanded_grade_idxs <= 63);
...
% Prepare the padded grade data once for the entire dataset
[all_padded_grades, old_to_new] = get_all_grades_with_padding(blind_pass_table,config);

% Find the padded columns corresponding to the two requested grades
grade1_row = find(cell2mat(old_to_new(:,1)) == grade_pair(1));
grade2_row = find(cell2mat(old_to_new(:,1)) == grade_pair(2));

grade1_cols = old_to_new{grade1_row,3};
grade2_cols = old_to_new{grade2_row,3};

grade_columns = [grade1_cols grade2_cols];

% Give every original row an ID so we can retrieve its grade data
% after the train/validation/test splits
blind_pass_table.original_row_id = (1:size(blind_pass_table,1)).';

% Store only the selected grade columns
...
%don't have to do every threshold
for i=1:length(thresholds)
    ...
    try
        ...
        val_data = selected_grade_data(val_table.original_row_id,:);        %rescale the validation data based on the training data

        disp("Training data size:")
        disp(size(training_data))
        
        disp("Validation data size:")
        disp(size(val_data))
