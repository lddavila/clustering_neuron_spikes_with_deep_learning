...
for i=1:length(sliced_table_of_best_rep)
    try
    ...
    for j=1:length(all_tetrodes_which_contain_best_ch)
        ...
        for k=1:size(all_combos,1)
            ...
            name_for_figure = "unit_"+string(current_data{1,"unit"})+"_tetrode_"+string(current_tetrode)+"_channels_"+string(channels_of_current_tetrode(all_combos(k,1)))+"_"+string(channels_of_current_tetrode(all_combos(k,2)))+".jpeg";
            f = figure;
