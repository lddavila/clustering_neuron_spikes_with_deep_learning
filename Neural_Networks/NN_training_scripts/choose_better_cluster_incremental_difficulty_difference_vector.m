function [] = choose_better_cluster_incremental_difficulty_difference_vector(varargin)
%the goal of this function is to train the neural network on progressively
%harder and harder challenges
%this is different from
%train_choose_better_cluster_incremental_difficulty.m because it expreses
%the cluster features as a vector of differences 
%this way the neural network learns based off the relationship between the
%left/right cluster instead of the features of the 2 clusters in isolation
...
dir_to_save_results_to = fullfile(parent_save_dir,"incremental_ch_bttr_nn_w_vector");
...
for i=1:size(permutations_table,1)
    ...
    cell_array_of_neural_networks{i} = dynamically_create_layers_for_nn(2219,permutations_table{i,"filter_sizes"},permutations_table{i,"num_layers"},2);
end
