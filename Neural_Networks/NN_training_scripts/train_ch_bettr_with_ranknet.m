function [] = train_ch_bettr_with_ranknet(varargin)
...
monitor = trainingProgressMonitor( ...
    Metrics=["TrainLoss","ValLoss"], ...
    XLabel="Iteration", ...
    Info="ExecutionEnvironment");
...
while iteration < numIterations && ~monitor.Stop
    ...
    [X1,X2,pairLabels] = getTwinBatch(training_all_comparisons,miniBatchSize,train_true_class,training_features_array);
    ...
    % Precompute validation labels & batch indices
    [valX1, valX2, valLabels] = getTwinBatch( ...
        val_all_comparisons, miniBatchSize, val_true_class, val_features_array);

    % Convert to dlarray (CB format)
    valX1 = dlarray(valX1,"CB");
    valX2 = dlarray(valX2,"CB");
    ...
    valLoss = crossentropy(Yval, valLabels,classificationMode="single-label");
