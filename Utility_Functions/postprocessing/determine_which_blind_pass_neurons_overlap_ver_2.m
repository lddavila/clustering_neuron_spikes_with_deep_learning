function [clusters_organized_by_same_group] = determine_which_blind_pass_neurons_overlap_ver_2(blind_pass_table,config)
...
for i=1:size(blind_pass_table,1)
    ...
    for j=i+1:size(blind_pass_table,1)
        current_cluster_group = clusters_organized_by_same_group{cluster_group_counter};
        ...
        for k=1:size(clusters_organized_by_same_group{cluster_group_counter},1)
            ...
            checks_that_must_be_passed(k) = index_of_max-1;

            print_status_iter_message("determine_which_blind_pass_neurons_overlap.m",[i,j],sum(~already_merged));
        end
        if all(checks_that_must_be_passed,"all")
