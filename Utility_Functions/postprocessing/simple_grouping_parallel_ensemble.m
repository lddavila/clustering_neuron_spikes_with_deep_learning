function [grouped_clusters] = simple_grouping_parallel_ensemble(blind_pass_table,config,track_faliures,options)
arguments
    blind_pass_table table                 %required
    config struct               %required
    track_faliures logical
    options.use_true_accuracy_instead_of_nn_filter logical = false % Optional named argument
    options.threshold_filter_for_true_accuracy double = 10;
end
...
if ~options.use_true_accuracy_instead_of_nn_filter
    ...
    elimination_condition = blind_pass_table{:,"accuracy"}<options.threshold_filter_for_true_accuracy;
    blind_pass_table(elimination_condition,:) = [];
end
