...
disp("Finished Importing Blind Pass Table and ground truth")

ground_truth = config.GT_FP;
...
for i=1:size(gradience_levels_to_add,2)
    ...
    for j=1:size(overlap_thresholds_to_try,2)
        ...
        for level_counter=1:size(list_of_levels,2)
            ...
            results_of_grouping = determine_which_blind_pass_neurons_overlap(table_to_group,config);
            new_row = analyze_cluster_group_contamination(results_of_grouping,config,ground_truth);
