...
channel_to_tetrode_dictionary = containers.Map('KeyType','char','ValueType','any');
spiking_channel_tetrode_dictionary = containers.Map('KeyType','char','ValueType','any');
channels_in_current_tetrode = art_tetr_array(1,:);
tetrode_dictionary("t"+string(1)) = channels_in_current_tetrode;
sorted_spike_windows_for_current_tetrode_dictionary = containers.Map('KeyType','char','ValueType','any');
for j=1:length(channels_in_current_tetrode)
    channel_to_tetrode_dictionary("c"+string(channels_in_current_tetrode(j))) = 1;
end
[current_slice,current_timing,current_spiking_channels,spike_slices_in_samples_format,sorted_spike_windows_for_current_tetrode] = get_slices_per_artificial_tetrode_ver_2(channels_in_current_tetrode,spike_windows,dir_with_chan_recordings,timestamps,number_of_dps_per_slice,scale_factor);



spike_tetrode_dictionary("t"+string(1)) = current_slice;
timing_tetrode_dictionary("t"+string(1)) = current_timing;
spiking_channel_tetrode_dictionary("t"+string(1)) = current_spiking_channels;
spike_tetrode_dictionary_samples_format("t"+string(1)) = spike_slices_in_samples_format;
sorted_spike_windows_for_current_tetrode_dictionary("t"+string(1)) = sorted_spike_windows_for_current_tetrode;
...
save(fullfile(dictionaries_dir,"t"+string(1)+" tetrode_dictionary.mat"),"-fromstruct",tetrode_dictionary);
save(fullfile(dictionaries_dir,"t"+string(1)+" spike_tetrode_dictonary.mat"),"-fromstruct",spike_tetrode_dictionary)
save(fullfile(dictionaries_dir,"t"+string(1)+" timing_tetrode_dictionary.mat"),"-fromstruct",timing_tetrode_dictionary)
save(fullfile(dictionaries_dir,"t"+string(1)+" channel_to_tetrode_dictionary.mat"),"-fromstruct",channel_to_tetrode_dictionary)
save(fullfile(dictionaries_dir,"t"+string(1)+" spiking_channel_tetrode_dictionary.mat"),"-fromstruct",spiking_channel_tetrode_dictionary)
save(fullfile(dictionaries_dir,"t"+string(1)+" spike_tetrode_dictionary_samples_format.mat"),"-fromstruct",spike_tetrode_dictionary_samples_format);
save(fullfile(dictionaries_dir,"t"+string(1)+" sorted_spike_windows.mat"),"-fromstruct",sorted_spike_windows_for_current_tetrode_dictionary);


status_message ="\n"+print_status_iter_message("get_dicationaries_of_all_spikes_ver_3.m",1,1);
fwrite(status_file,status_message);
fclose(status_file);
end