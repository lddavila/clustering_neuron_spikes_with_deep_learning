...
if ~isempty(varargin)
    save_name = "stage_"+config.plot_counter+"ClustLvl"+string(which_clustering_level)+"subset"+string(config.which_subset)+varargin{1}+"_"+strjoin(string(config.percentiles_to_use),"");
else
    save_name = "stage_"+config.plot_counter+"ClustLvl"+string(which_clustering_level)+"subset"+string(config.which_subset)+"_"+strjoin(string(config.percentiles_to_use),"");
end
...
if ~isfile(fullfile(current_mult_dir,save_name+".png"))
    ...
    for i=1:size(all_dim_combos,1)
        ...
        for j=1:length(cluster_idx_struct)
            ...
            raw_overlap_number = zeros(length(unique_gt_units),1);
            raw_missing_number = zeros(length(unique_gt_units),1);
            ...
            for k=1:length(unique_gt_units)
                ...
                raw_overlap_number(k) = sum(is_member_tol_result);
                raw_missing_number(k) = length(current_idx) - raw_overlap_number(k);
            end
            ...
            if i==size(all_dim_combos,1)
                legend_string = [legend_string,"Cluster "+string(j)+" #spikes: "+string(length(current_idx))];
            ...
            for k=1:length(unique_gt_units)
                if all_overlaps(k) > .999
                    ...
                    if i==size(all_dim_combos,1)
                        local_name =  "unit: " +string(unique_gt_units(k))+ " detection ratio: " + string(all_overlaps(k) +" unit spikes:"+string(raw_overlap_number(k)));
