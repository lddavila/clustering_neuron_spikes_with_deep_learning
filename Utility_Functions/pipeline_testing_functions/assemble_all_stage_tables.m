...
all_stage_tables.tetrode = split_folder(:,3);
...
for i=1:height(grouped_table)
    ...
    if ~isempty(setdiff(current_vars,overall_list_of_vars)) && i~=1 %the current table has variables the running table does not
    ...
    elseif ~isempty(setdiff(overall_list_of_vars,current_vars)) && i~=1 %the running table has variables the current table does not
