...
save_img_dir = create_a_file_if_it_doesnt_exist_and_ret_abs_path(fullfile(config.parent_save_dir,"clustering_toy_data_plots"));
...
for i=1:size(config.ART_TETR_ARRAY,1)
    current_peaks = abs(toy_data(:,config.ART_TETR_ARRAY(i,:))); %get the peaks for the current tetrode
    ...
    config.current_channels = config.ART_TETR_ARRAY(i,:);
    ...
    for k=1:size(all_comps,1)
        ...
        for j=1:length(clusters_for_toy_data{i})
            ...
            scatter(x_data(current_cluster_idxs),y_data(current_cluster_idxs),3,cluster_colors(j,:),'filled');
            hold on;
        end
        xlabel("simulated dimension "+string(current_channels(dim_1)) +" (in \muV)")
        ylabel("simulated dimension "+string(current_channels(dim_2)) +" (in \muV)")
        ...
        save_plots_in_all_formats(f,fullfile(art_tetr_dir,"Tetrode "+string(i)));
        close(f);
    end
