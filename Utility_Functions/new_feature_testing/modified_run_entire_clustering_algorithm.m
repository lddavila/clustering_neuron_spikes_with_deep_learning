function [meets_acc_ratio] = modified_run_entire_clustering_algorithm(config)
%this version differs from the mainline production version in that it will
%smartly navigate all default z scores in order to find which z score
%between 3 and 4 will maximize the ratio of clusters that have at least
%70% accuracy
%the z scores that it will use will be determined by a learning agent??
...
for min_z_score = z_scores_to_check
    ...
    %accuracy
    meets_acc_ratio =sum(blind_pass_table{:,"accuracy"}>=accuracy_threshold) /size(blind_pass_table,1);
