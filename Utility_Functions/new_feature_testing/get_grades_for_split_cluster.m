function [blind_pass_table] = get_grades_for_split_cluster(blind_pass_table,config,options)
arguments
...
grading_error_dir = create_a_file_if_it_doesnt_exist_and_ret_abs_path(fullfile(config.error_dir,"grading_errors_for_split_clusters"));
...
print_status_bar(num_iterations,"get_grades_for_split_cluster.m")
...
for i=1:size(sliced_blind_pass_table,1)
    ...
    grades_fp = repelem(fullfile(dir_to_save_grades_to,current_tetrode+" Grades.mat"),height(current_data),1);
    current_data.grades = grades;
    current_data.grades_fp = grades_fp;
    grades_table{i} = current_data;
    send(q,[])
