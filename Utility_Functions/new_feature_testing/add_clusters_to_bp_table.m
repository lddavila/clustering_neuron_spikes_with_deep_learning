...
sliced_bp_table = slice_table_for_parallel_processing(blind_pass_table,["fp_to_cleaned_clusters"]);
...
for i=1:size(sliced_bp_table,1)
    ...
    idx_cell_array = cell(length(cleaned_clusters),1);
    timestamp_cell_array = cell(length(cleaned_clusters),1);
    ...
    current_new_data = table(repelem(current_data{1,"fp_to_cleaned_clusters"},length(idx_cell_array),1),(1:length(idx_cell_array)).',idx_cell_array,timestamp_cell_array,'VariableNames',["fp_to_cleaned_clusters","Cluster","cluster_idx","timestamps"]);
