function [the_clusters] = get_clusters_for_toy_data(the_subsets,peaks)
  % disp("finished core cluster loop")
    ...
    start_cl.idx = 1:size(peaks, 1); % put all spikes in spike_aligned into the initial cluster 
    ...
    the_clusters = cellmap(@(x) x.idx, clusters); %returns the result of all subclustering
