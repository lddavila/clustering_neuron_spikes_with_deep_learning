function [agent,critic,obs_info,action_info] = get_agent_and_critique_net_for_finding_z_score_with_img(num_features,num_neurons_per_layer,num_layers,epsilon_num)

obs_info = rlNumericSpec([1,num_features]);
action_info = rlFiniteSetSpec([1,-1,0]); 
%-1:    maps to decrease the z score and get get new image
% 1:    maps to increase the z score and get the new image
% 0:    maps to run the clustering to evaluate your rewards
