...
fp_to_dir = "C:\Users\ldd77\OneDrive\Desktop\clustering_neuron_spikes_with_deep_learning\Default_Results_Dir\img_threshold_finding_incremental\accuracy_text_files\*.txt";
home_dir = cd("..");
cd("..");
addpath(genpath(pwd));
cd(home_dir);
...
parfor i=1:length(sliced_txt_files)
    ...
    current_data_string = split(string(current_data{1,"name"}),"_",2);
...
par_save(fullfile("C:\Users\ldd77\OneDrive\Desktop\clustering_neuron_spikes_with_deep_learning\Default_Results_Dir\img_threshold_finding_incremental","table_of_image_accuracy_data.mat"),final_table);
end