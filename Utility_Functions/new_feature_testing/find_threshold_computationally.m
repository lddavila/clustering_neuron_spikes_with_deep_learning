...
legend_string = [];
figure;
for k=1:num_tol_to_try
    for i=1:1.5%height(table_of_channels)
        ...
        [ironclust_spike_det_results,ironclust_noise_ratio,ironclust_raw_unit_numbers,ironclust_raw_noise_numbers,ic_thresholds]= get_detected_spikes(current_channel_data,default_thresholds,ironclust_thresholds,unit_gr_tr(table_of_distance.Unit),config,fp_to_save_ironclust_images,tol);
        ...
        plot(ic_thresholds,ironclust_spike_det_results(1,:))
        xlabel("Multiplier");
        ylabel("Signal Ratio");
        legend_string = [legend_string,sprintf("%.10f",tol)];
