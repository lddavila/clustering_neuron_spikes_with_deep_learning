...
table_of_stats = cell2table(cell(0,5),'VariableNames',["sorter","#_clusters_found","#_mua_clusters_created","#_missed_gt_units","missed_gt_units"]);
groupcounts_of_bp = groupcounts(blind_pass_table,"Max_Overlap_Unit");
missed_gt_units = setdiff(1:num_gt_units,groupcounts_of_bp{:,"Max_Overlap_Unit"});
...
current_row = table("ours",size(blind_pass_table,1),sum(blind_pass_table{:,"accuracy"}<min_accuracy),length(missing_gt_units),{missing_gt_units}, ...
    'VariableNames', ...
    ["sorter","#_clusters_found","#_mua_clusters_created","#_missed_gt_units","missed_gt_units"]);
...
for k=1:length(comparisons)
    ...
    current_row = table(string(spike_sorter{end}),size(other_table_agreement_matrix,1), ...
        ...
        length(list_of_gt_units_missed),{list_of_gt_units_missed},...
        'VariableNames', ...
        ["sorter","#_clusters_found","#_mua_clusters_created","#_missed_gt_units","missed_gt_units"]);
    table_of_stats = [table_of_stats;current_row];
