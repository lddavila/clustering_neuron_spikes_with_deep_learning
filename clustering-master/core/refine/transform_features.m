...
function the_scaled_features = transform_features(the_features, the_cluster_idx)
...
%   the data in that feature alone.
    ...
    feature_filter = find_singular_cols(the_features(the_cluster_idx, :));
    filtered_features = the_features(:, feature_filter);
    ratings = 1 - rate_clusters({the_cluster_idx}, filtered_features)';
    good_ratings = ratings > 0;
    the_scaled_features = filtered_features(:, good_ratings) .* repmat(ratings(good_ratings), [num_spikes, 1]);
end