...
function the_final_grades = compute_gradings(the_aligned, the_timestamps, the_tvals, the_clusters, the_config)
...
%   10) Bhattacharyya Distance to unsorted spikes (useless because we simply have too many spikes for this to be useful)
    ...
    the_final_grades = nan(num_clusters, 27);
    total_raw_spikes = 1:size(the_aligned, 2);
    all_peaks = get_peaks(the_aligned, true);
    temp = load('template.mat');
    for k = 1:num_clusters
        % Set up cluster-specific vars
        cluster_filter = the_clusters{k};
        ts = the_timestamps(cluster_filter);
        spikes = the_aligned(:, cluster_filter, :);
        ...
        wire_thresh = the_tvals(compare_wire);
        ...
        the_final_grades(k, 1) = lratio;
        
        % Peak cv check
        cv = compute_cv(peaks);
        the_final_grades(k, 2) = cv;
        
        % ISI check
        isi = diff(ts) * 1e-6; % Convert to seconds
        short_isi_len = the_config.params.GR_SHORT_ISI_LEN;
        short_isi = sum(isi < short_isi_len)/length(isi); % Fraction of ISI < short_isi_len
        the_final_grades(k, 3) = short_isi;
        
        % Theoretical fraction below threshold
        below_threshold = compute_incompleteness(compare_peaks, wire_thresh);
        the_final_grades(k, 4) = below_threshold;
        
        % Isolation distance
        the_final_grades(k, 5) = mahal(double(wire_thresh), compare_peaks');
        
        % Number of spikes
        the_final_grades(k, 6) = length(cluster_filter);
        
        % Stationarity
        t_mu = mean(the_timestamps);
        t_std = std(the_timestamps);
        cluster_med = median(the_timestamps(cluster_filter));
        the_final_grades(k, 7) = cluster_med < t_mu - t_std || cluster_med > t_mu + t_std;
        ...
        if length(cluster_filter) > 1
            ...
            the_final_grades(k, 8) = template_match(mean_waveform, temp.nt);
        else
            the_final_grades(k, 8) = 0;
        end
        ...
        min_bhat = min(dists);
        the_final_grades(k, 9) = min_bhat;
        
        % Bhat distance to unsorted
        other_cf = setdiff(1:size(all_peaks, 2), unique(vertcat(the_clusters{:})));
        other_peaks = all_peaks(:, other_cf)';
        dim_filt = find_singular_cols(peaks) & find_singular_cols(other_peaks);
        if ~isempty(other_peaks) && any(dim_filt)
            the_final_grades(k, 10) = bhat_dist(peaks(:, dim_filt), other_peaks(:, dim_filt));
        end
        
        the_final_grades(k, 11) = compute_lratio(peaks(:, dim_filt), other_peaks(:, dim_filt));
        
        rep_wire = shiftdim(spikes(compare_wire, :, :), 1);
        [~, snr] = compute_new_cv(rep_wire, 0.5);
        the_final_grades(k, 16) = snr;
        [~, snr] = compute_new_cv(rep_wire, 0.33);
        the_final_grades(k, 17) = snr;
        [~, snr] = compute_new_cv(rep_wire, 0.25);
        the_final_grades(k, 18) = snr;
        the_final_grades(k, 12) = compute_new_cv(rep_wire, 0.5);
        the_final_grades(k, 13) = compute_new_cv(rep_wire, 0.33);
        the_final_grades(k, 14) = compute_new_cv(rep_wire, 0.25);
        
        isi = isi * 1e3; % milliseconds
        the_final_grades(k, 15) = sum(isi < 7.5) / sum(isi < 100);
        
        if isempty(other_cf)
            near_thresh_idx = [];
        else
            near_thresh_idx = other_cf(all(bsxfun(@(x, y) x < y, other_peaks, 1.5 * the_tvals), 2));
        end
        ...
        if sum(dim_filt) > 1 && length(near_thresh_idx) > 0.5*length(cluster_filter)
            the_final_grades(k, 19) = bhat_dist(peaks(:, dim_filt), near_thresh_peaks(:, dim_filt));
        end
        ...
        if any(dim_filt) && ~isempty(other_cf)
            ...
            if length(near_clust_idx) > 0.2 * length(cluster_filter)
                ...
                the_final_grades(k, 20) = bhat_dist(peaks(:, dim_filt), near_clust_peaks(:, dim_filt));
            else
                the_final_grades(k, 20) = Inf;
            end
            the_final_grades(k, 21) = length(near_clust_idx);
        end
        
        cluster_t = the_timestamps(cluster_filter);
        duration = 2 * std(cluster_t);
        the_final_grades(k, 22) = duration / (the_timestamps(end) - the_timestamps(1));
        
        mean_spike = shiftdim(mean(spikes(compare_wire, :, :)), 1);
        if length(cluster_filter) > 1
            [~, ~, mpc] = hfcm(peaks, 2, the_config);
            the_final_grades(k, 23) = mpc;
            dim_filt = find_singular_cols(peaks, 0.5);
            the_final_grades(k, 24) = sum(dim_filt);
            ...
            the_final_grades(k, 25) = dur;
        else
            the_final_grades(k, 23) = 0;
            the_final_grades(k, 24) = 0;
            the_final_grades(k, 25) = 0;
        end
        
        the_final_grades(k, 26) = length(cluster_filter) * 1e6 / (the_timestamps(end) - the_timestamps(1));
        ...
        the_final_grades(k, 27) = has_valley;
    end
end